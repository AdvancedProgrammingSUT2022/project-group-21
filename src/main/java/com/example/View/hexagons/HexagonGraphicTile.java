...
public class HexagonGraphicTile extends Polygon {
    ...
    public HexagonGraphicTile(double x, double y, int j, int i, Tile tile) {
        ...
        getPoints().addAll(x, y, x, y + r, x + n, y + r * 1.5, x + TILE_WIDTH, y + r, x + TILE_WIDTH, y, x + n, y - r * 0.5);
        setFill(new ImagePattern(new Image(getClass().getResource("/Terrain/" + tile.getTerrain().name().toLowerCase() + ".png").toExternalForm())));
        setOpacity(0.7);
        setStrokeWidth(1);
    ...
    public void update(Civilization civilization) {
        ...
        if (civilization.isTileRevealed(tile.X, tile.Y)) fogOfWar.setVisible(false);
        else fogOfWar.setVisible(true);
               if (tile.getOwner()!=civilization) this.setOpacity(0.3);
