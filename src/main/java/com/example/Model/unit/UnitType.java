...
public enum UnitType {
	...
	CATAPULT(CombatType.SIEGE, 100, 4, 14, 2, 2, Resource.IRON,
			...
			new UnitAbility[] { UnitAbility.BONUS_VS_CITY_10, UnitAbility.NO_DEFENSIVE_BONUS,
					UnitAbility.LIMITED_VISIBILITY }),
	...
	ARTILLERY(CombatType.SIEGE, 420, 16, 32, 3, 2, null,
			...
			new UnitActionType[] { UnitActionType.DELETE, UnitActionType.MOVE, UnitActionType.SLEEP_WAKE,
					UnitActionType.ALERT, UnitActionType.FORTIFY, UnitActionType.PILLAGE,
					UnitActionType.RANGE_ATTACK, UnitActionType.PRE_ATTACK_SETUP },
			new UnitAbility[] { UnitAbility.INDIRECT_FIRE, UnitAbility.BONUS_VS_CITY_10,
					UnitAbility.NO_DEFENSIVE_BONUS }),
	...
	PANZER(CombatType.ARMORED, 450, 60, -1, -1, 5, null,
			...
			new UnitActionType[] { UnitActionType.DELETE, UnitActionType.MOVE, UnitActionType.SLEEP_WAKE,
					UnitActionType.ALERT, UnitActionType.FORTIFY, UnitActionType.PILLAGE,
					UnitActionType.MELEE_ATTACK },
			new UnitAbility[] { UnitAbility.NO_DEFENSIVE_BONUS, UnitAbility.LIMITED_VISIBILITY,
					UnitAbility.CAN_MOVE_AFTER_ATTACK }),
	...
	TANK(CombatType.ARMORED, 450, 50, -1, -1, 4, null,
			...
			new UnitActionType[] { UnitActionType.DELETE, UnitActionType.MOVE, UnitActionType.SLEEP_WAKE,
					UnitActionType.ALERT, UnitActionType.FORTIFY, UnitActionType.PILLAGE,
					UnitActionType.MELEE_ATTACK },
			new UnitAbility[] { UnitAbility.PENALTY_VS_CITY_10, UnitAbility.NO_DEFENSIVE_BONUS,
					UnitAbility.CAN_MOVE_AFTER_ATTACK });
	...
	public final ImagePattern imagePattern;

	private UnitType(CombatType combatType, int cost, int combatStrength, int rangedCombatStrength, int range, int MP, Resource resourceRequired, Technology technologyRequired[], UnitActionType actionTypes[], UnitAbility unitAbilities[]) {
	...
	private ImagePattern getImagePattern(){
		Image image = null;
		try {
			image = new Image(App.class.getResource("assets/Unit/" + toString().toLowerCase() + ".png").toExternalForm());
		} catch (Exception e) {
			System.out.println("Error: cant load image file: " + toString());
			e.printStackTrace();
		}
	...
	public boolean canBuildOnCity(City city) {
		if (this == SETTLER && city.countCitizens() < 2)
			return false; // SETTLER can be made at city with at least 2 citizens
		if (!city.getOwner().hasTechnologies(technologyRequired))
			return false;
		if (resourceRequired != null && !city.hasImprovedResource(resourceRequired))
			return false;
		return true;
	}
